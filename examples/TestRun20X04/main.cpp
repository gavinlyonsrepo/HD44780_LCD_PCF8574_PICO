 ...
 * (2) This is for 20 column 4 row LCD 
 */
...
int main()
{
  ...
  myLCD.PCF8574_LCDInit(LCDCursorTypeOn,4 ,20);
...
void gotoTest(void) {
  ...
  for (rowPos = 1; rowPos < 5; rowPos++)
  {
    ...
    {
        myLCD.PCF8574_LCDGOTO((LCDLineNumber_e)rowPos, columnPos);
...
void clearLineTest(void)
{
    uint8_t rowPos = 1;
     for (rowPos = 1; rowPos < 5; rowPos++)
     {
        myLCD.PCF8574_LCDClearLine((LCDLineNumber_e)rowPos);
        busy_wait_ms(DISPLAY_DELAY_2);
     }
}
